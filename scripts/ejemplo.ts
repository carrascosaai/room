...
const CAST: BotSpec[] = [
  ...
  { id: "p_ana", nickname: "Ana", lang: "es", traits: { ...base, conformity: 0.9, socialAlignment: 0.85, contrarianism: 0.1 } },
  ...
  { id: "p_mar", nickname: "María", lang: "es", traits: { ...base, patience: 0.9, consistency: 0.85, risk: 0.15, contrarianism: 0.6 } },
];
...
function optLabel(state: GameState, round: NonNullable<ReturnType<typeof currentRound>>, pid: string, oid: string): string {
  ...
  if (!o) return oid;
  // player-target option → show the player's name
...
function run(seed: number) {
  ...
  while (state.phase !== "FINAL_RESULTS" && guard++ < 300) {
    ...
    if (state.phase === "ANSWERING" && round) {
      ...
      if (outcome) {
        for (const l of outcome.lines) console.log(`      → ${l.es}`);
      }
      flushAi();
      continue;
    }
  ...
  for (const s of r.superlatives) {
    console.log(`  ${label[s.key]?.padEnd(20)} → ${s.playerId ? NAME[s.playerId] : "—"}`);
  }
  if (r.biggestAlliance) console.log(`  ${"La mayor alianza".padEnd(20)} → ${NAME[r.biggestAlliance.a]} + ${NAME[r.biggestAlliance.b]}`);
  if (r.biggestBetrayal) console.log(`  ${"La mayor traición".padEnd(20)} → ${NAME[r.biggestBetrayal.from]} → ${NAME[r.biggestBetrayal.to]}`);
  console.log(`  ${"Precisión de la IA".padEnd(20)} → ${Math.round(r.aiAccuracy * 100)}%  (teorías: ${r.theoriesHeld}/${r.theoriesTested} acertadas)`);
