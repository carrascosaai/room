  ...
  return [
    {
      ...
      content: [
        ...
        `Say the same thing again in very simple ${infoOf(character).name} (A2 level): short sentences, very common words.`,
...
export function buildSuggestionMessages(level: Level, lastReply: string, context: Turn[], lang: TargetLang = "en"): ChatMessage[] {
...
export function buildTranslateMessages(text: string, lang: TargetLang = "en"): ChatMessage[] {
  const L = LANGS[lang];
  return [
    {
      role: "system",
